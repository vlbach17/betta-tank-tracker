...
- **Usability review of the persistent bottom nav.** `NavChips` was itself replaced by a fixed bottom tab bar (Now/Overview/Log/History/Settings) — a deliberate usability pass on *this* pattern (not the superseded `NavChips` one) still hasn't been done.
