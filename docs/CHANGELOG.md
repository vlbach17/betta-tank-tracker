...
- Replaced Dashboard's header text links ("Overview"/"Settings") with a kebab menu button plus a pill-style `NavChips` row, also added to Overview and Settings (both drop `BackLink` in favor of it — a navigation-structure change, not just a restyle); `NavChips` was later replaced by a persistent `BottomNav` bar shared across Dashboard, Overview, Settings, and History
- New History screen (`/history`) listing every logged test session reverse-chronologically with an overall status pill, and an Entry Detail screen (`/history/:testedAt`) showing every reading from that session with per-reading delete
