...
### Added
- Water Changes tab in Tank Info — logs date, amount (gallons), and optional notes per water change to a new `water_changes` table via a modal ("Log a water change"); reverse-chronological list with confirm-before-delete (mirroring `ParameterHistory`'s reading-delete pattern), unlike the other Tank Info tabs it has no reorder/archive since it's a chronological log, not a managed entity list
- Tank Info page (`/tank-info`, linked from Settings) — 6 tabs (Equipment, Food, Plants, Fish, Hardscape, Water Treatment) each backed by its own Supabase table with full CRUD (add, edit, delete, up/down reorder, active/inactive toggle), mirroring Parameters' existing pattern; equipment/food/plants/hardscape/water-treatment items can link out to an external product URL and show a purchase date, fish rows show the existing Spunk avatar when the name matches. Add and Edit share one bottom-sheet-on-mobile modal with an explicit Category selector — picking a category isn't inferred from whichever tab you're on, and changing an item's category on Edit moves it to that table (create-then-delete, so a failure partway through leaves a visible duplicate rather than losing data)
- Tank Info's tab strip hides its native horizontal scrollbar and adds scroll-snap plus an edge-fade mask, so dragging/swiping between its 7 tabs reads as an intentional gesture instead of a stray scrollbar; the active tab auto-scrolls into view on selection
- Imported historical tank purchases from a CSV into Tank Info (Equipment/Plants/Food/Hardscape/Water Treatment); 4 rows were skipped as duplicates of items already entered manually
